...
from verl.workers.fsdp_workers import create_device_mesh, get_sharding_strategy, CriticWorker, RewardModelWorker, ActorRolloutRefWorker


# """
# below is the original import from verl/verl/workers/fsdp_workers.py
# Ideally, we want to inherit from the verl worker, but the implementation will raise this error: the worker class should be the same when share the same process. We avoid this error by implementing without inheritance. We hope to find a better solution in the future.
# """
...
#         assert vllm_mode == 'customized', f"vllm_mode not supported in RAGEN: {vllm_mode}"
#         from ragen.workers.rollout.vllm_rollout import vLLMRollout
...
#     # below is the same as VerlActorRolloutRefWorker
