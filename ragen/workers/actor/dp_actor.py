        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    if self.config.use_kl_loss:
                        if not self.config.use_ref_policy:
                            kl_loss = core_algos.kl_penalty(logprob=log_prob, ref_logprob=old_log_prob, kl_penalty=self.config.kl_loss_type)
                            kl_loss = masked_mean(kl_loss, response_mask)
                        else:
